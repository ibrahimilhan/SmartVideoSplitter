...
class SmartVideoSplitterApp:
    ...
    def setup_fonts(self):
        ...
        self.font_log = ("Cascadia Code", 10)
        self.font_overlay = ("Segoe UI", 32, "bold")
        self.font_warning_title = ("Segoe UI", 11, "bold")
        self.font_small = ("Segoe UI", 9)
        self.font_stat = ("Segoe UI", 26, "bold")
    ...
    def setup_ui(self):
        # Ana container
        main_container = tk.Frame(self.root, bg=COLORS["bg_dark"], padx=24, pady=16)
        ...
        if os.path.exists(banner_path):
            try:
                ...
                # Resmin ortasindan ince uzun panoramik (banner) formatinda kirp
                img_w, img_h = img.size
                target_w, target_h = 950, 160
                
                # Goruntunun merkezini bulup yatayda tam, dikeyde ortadan kirpalim
                ...
                # Referansi sinifta sakla ki cop toplayici (GC) silmesin
                self.banner_img = ImageTk.PhotoImage(img_resized)
                
                banner_lbl = tk.Label(main_container, image=self.banner_img, bg=COLORS["bg_dark"], bd=0)
                banner_lbl.pack(fill=tk.X, pady=(0, 16))
        ...
        tk.Label(title_frame, text="Smart Video Splitter",
        ...
        ver_frame = tk.Frame(header, bg=COLORS["accent2"], padx=10, pady=4)
        ver_frame.pack(side=tk.RIGHT, anchor="ne")
        tk.Label(ver_frame, text="v1.0", font=self.font_small,
                 fg=COLORS["text_primary"], bg=COLORS["accent2"]).pack()
        ...
        tk.Button(btn_frame, text="🔄 Sıfırla", bg=COLORS["bg_card_alt"], fg="white",
                  font=self.font_body_bold, padx=14, pady=6, relief=tk.FLAT, cursor="hand2",
                  activebackground=COLORS["bg_card"], activeforeground="white",
                  command=self._reset_ui).pack(side=tk.RIGHT)
    ...
    def _reset_ui(self):
        ...
        self.entry_q_count.delete(0, tk.END)
